          ...
          <Card>
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <p className="p-6 text-center text-gray-500 dark:text-gray-400">No parts found.</p>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                    ...
                    {parts.map((part: {
                      ...
                      <TableRow key={part.id}>
                        ...
                        <TableCell className="hidden sm:table-cell">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">{part.job.customer.firstName} {part.job.customer.lastName}</p>
                        </TableCell>
            ...
            {vendors?.map((v: { id: string; name: string; contactName?: string; phone?: string; email?: string; accountNum?: string }) => (
              <Card key={v.id}>
                <CardContent className="p-4 text-sm space-y-1">
                  ...
                  {v.contactName && <p className="text-gray-600 dark:text-gray-400">{v.contactName}</p>}
                  {v.phone && <a href={`tel:${v.phone}`} className="text-blue-600">{v.phone}</a>}
                  {v.email && <p className="text-gray-500 dark:text-gray-400">{v.email}</p>}
                        ...
                        {marginData.byVendor.map((v: { name: string; revenue: number; cost: number; count: number; marginPct: number }) => (
                          <div key={v.name}>
                            <div className="flex justify-between text-sm mb-1">
                              ...
                              <span className="text-gray-500 dark:text-gray-400">{formatCurrency(v.revenue)} rev · {v.count} parts · <span className={`font-bold ${v.marginPct >= 30 ? "text-green-600" : v.marginPct >= 15 ? "text-yellow-600" : "text-red-600"}`}>{v.marginPct}%</span></span>
                            </div>
