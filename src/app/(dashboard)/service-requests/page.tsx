  ...
  return (
    <div className="space-y-6">
      ...
      {pending.length === 0 && others.length === 0 && (
        <Card>
          <CardContent className="p-8 text-center text-gray-500">No service requests yet.</CardContent>
        </Card>
      )}

      {pending.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">New Requests</h2>
          {pending.map((r: {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="bg-gray-50 rounded-md p-3">
                      <p className="text-xs font-medium text-gray-500 mb-1">{r.serviceType}</p>
      ...
      {others.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">Previous Requests</h2>
          {others.map((r: { id: string; name: string; phone: string; serviceType: string; status: string; createdAt: string }) => (
            <div key={r.id} className="flex items-center justify-between p-3 bg-white rounded-lg border text-sm">
              <div>
                ...
                <span className="text-gray-600">{r.serviceType}</span>
                <span className="text-gray-400 ml-2 text-xs">{formatDate(r.createdAt)}</span>
              </div>
              <span className={`text-xs rounded-md px-2 py-0.5 font-medium ${STATUS_COLORS[r.status] || "bg-gray-100 text-gray-700"}`}>
                {r.status}
              </span>
            </div>
          ))}
